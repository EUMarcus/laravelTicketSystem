...
class SuggestionSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Get a random profile for posted_by (or use null for anonymous)
        ...
        foreach ($suggestions as $index => $suggestionData) {
            try {
                // Randomly assign a profile or leave as null (anonymous)
                $postedBy = !empty($profileIds) && rand(0, 1) ? $profileIds[array_rand($profileIds)] : null;

                Suggestion::create([
                    'id' => (string) Str::uuid(),
                    'title' => $suggestionData['title'],
                    'category' => $suggestionData['category'],
                    'full_content' => $suggestionData['full_content'],
                    'status' => $suggestionData['status'],
                    'posted_by' => $postedBy,
                ]);

                $this->command->info('✓ Created: ' . $suggestionData['title']);
            } catch (\Exception $e) {
                $this->command->error('✗ Failed to create: ' . $suggestionData['title']);
